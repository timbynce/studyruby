...
def seed
  ...
  @trains << Pass_Train.new
  @trains << Cargo_Train.new
  @wagons << Pass_Wagon.new
  @wagons << Pass_Wagon.new
  @wagons << Cargo_Wagon.new
end
...
def create_train
  ...
  number = gets.chomp
  if type == "passenger"
    @trains << Pass_Train.new(number)    
  elsif type == "cargo"
    @trains << Cargo_Train.new(number)    
  else
